...
def cleansing_and_plot():
    ...
    # ----i drop this df becasue after the investigation in step 1 i saw the most importnat column is 70% empty
...
# -----------total wins for top teams  plot------------------------------------------------------
    ...
    plt.title('Total Wins for Top  Teams (Filtered Data)')
...
# --away wins for top teams plot-------------------------------------------------
    ...
    plt.title('Wins Away for Top  Teams (Filtered Data)')  
...
#  i created a df with team and minute group by a team 
...
# --------------------------------------------------Goals Per Minutes for Team plot 
    ...
    for team, team_data in merged_df1.groupby('team'):
        plot_goal_distribution(team_data.copy(), team) 
...
# plot for team stat mean minute for total per team 
